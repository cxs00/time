...
if [ ! -f "src/html/interface-showcase.html" ]; then
    print_colored "$RED" "❌ 界面展示文件不存在: src/html/interface-showcase.html"
    exit 1
fi

print_colored "$GREEN" "✅ 界面展示文件存在"
print_colored "$WHITE" ""

# 直接打开文件
print_colored "$BLUE" "🚀 打开界面展示页面..."

if command -v open &> /dev/null; then
    # macOS
    open "src/html/interface-showcase.html"
    print_colored "$GREEN" "✅ 已在浏览器中打开界面展示页面"
    ...
    print_colored "$WHITE" "  文件路径: $(pwd)/src/html/interface-showcase.html"
fi
